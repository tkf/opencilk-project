...
bool TailDuplicator::tailDuplicate(bool IsSimple, MachineBasicBlock *TailBB,
  ...
  // This optimization is intended to fix up poor code generated by
  // builtin setjmp, which is expanded after most optimizations have run.
  Register Reg;
  bool Dead = false;
  MachineBasicBlock *Zero = nullptr, *Nonzero = nullptr;
  bool BRNZ =
    !PreRegAlloc && TII->isZeroTest(*TailBB, Reg, Dead, Zero, Nonzero);
  int64_t PredValue = 0;
  ...
  for (MachineBasicBlock *PredBB : Preds) {
    ...
    /* If setsRegister(Dead = true) returns true the predecessor block
       has been modified and we are committed to merging the two blocks. */
    bool Simplifiable =
      BRNZ && TII->setsRegister(*PredBB, Reg, Dead, PredValue);
    ...
    if (IsLayoutSuccessor && !Simplifiable)
      continue;
